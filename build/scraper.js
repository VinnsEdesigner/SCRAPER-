...
`),document.head.appendChild(e)}var qe=v(()=>{Ie();Te();Be();Ce();Ne();je();Pe();Re()});function Ye(){De();let e=document.createElement("div");e.id=Ue,e.innerHTML='\u{1F577}\uFE0F<span class="vn-fab-dot"></span>';let t=document.createElement("div");return t.id=te,t.innerHTML=`
  ...
  `}var Vt,g,S,A,C=v(()=>{f();le();Y();q();L();Vt="https://vinnsedesigner-vinns-ai-backend.hf.space",g=null,S=[],A=new Map});f();L();ie();f();D();var Ke="https://vinnsedesigner-vinns-ai-backend.hf.space",Nt=10,Mt=2e3,jt=2048,Ot=3e4,Pt=6e4,Ht=20;function Rt(e){return!e||typeof e!="string"?!1:e.trimStart().startsWith("data:")}function Dt(e){let t=typeof e.text=="string"?e.text:"";return Rt(t)?(console.warn("[VENOM mini-agent] base64 stripped from snippet before send",{type:e.type}),e.type==="image"&&typeof e.url=="string"?e.url.slice(0,jt):""):t.slice(0,Mt)}function qt(e){if(!e||typeof e!="object"||!new Set(["code","research","image","file"]).has(e.type))return null;let n=Dt(e);return!n&&e.type!=="image"||!n&&e.type==="image"&&!e.url?null:{type:e.type,text:n||e.url||""}}function Je(e){return!Array.isArray(e)||e.length===0?[]:e.slice(0,Ht).map(qt).filter(Boolean)}function Ze(e){if(!Array.isArray(e)||e.length===0)return[];let t=new Set(["user","assistant","system"]);return e.filter(n=>n&&t.has(n.role)&&n.content).slice(-Nt).map(n=>({role:n.role,content:String(n.content).slice(0,1e3)}))}function Ut(){return u()||null}function Yt(){try{return j()}catch(e){return console.warn("[VENOM mini-agent] buildPageContext failed",e.message),{url:typeof window<"u"?window.location.href:"",title:typeof document<"u"?document.title:"",content:""}}}async function Xt(e,t,n,o,r,s){try{let i=await fetch(`${Ke}/api/agent`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${o}`},body:JSON.stringify({message:e,pageContext:n,history:Ze(r),snippets:Je(t),sessionId:s||null,context:"bookmarklet",stream:!1}),signal:AbortSignal.timeout(Pt)}),a=await i.json();return i.ok?{reply:a.reply||a.response||"",model:a.model_used||a.model||"agent",escalated:!0,tools_used:a.tools_used||[],searched:!1}:{error:a.message||`Full agent error (${i.status})`,escalated:!0}}catch(i){return{error:i.name==="TimeoutError"||i.name==="AbortError"?"Full agent timed out \u2014 try again":"Full agent unreachable",escalated:!0}}}async function ae(e,t=[],n=[],o={}){let{sessionId:r=null,preferredModel:s=null}=o,i=Ut();if(!i)return{error:"Not authenticated \u2014 open dashboard and login first"};if(!e||typeof e!="string"||!e.trim())return{error:"Message is required"};let a=e.trim(),l=Yt(),b=Je(t),$=Ze(n),P={message:a,pageContext:l,history:$,snippets:b,sessionId:r||null,preferredModel:s||null},E,k;try{k=await fetch(`${Ke}/api/lite-agent`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${i}`},body:JSON.stringify(P),signal:AbortSignal.timeout(Ot)}),E=await k.json()}catch(Se){return{error:Se.name==="TimeoutError"||Se.name==="AbortError"?"Lite agent timed out \u2014 check connection":"Backend unreachable \u2014 check connection"}}return k.ok?E.escalate===!0?Xt(a,t,l,i,n,r):{reply:E.reply||"",model:E.model||"lite",escalated:!1,tools_used:E.tools_used||[],searched:E.searched||!1}:k.status===503?{error:"All AI providers are currently unavailable \u2014 try again shortly"}:k.status===429?{error:E.message||"Rate limit reached \u2014 try again in a few minutes"}:k.status===401?{error:"Session expired \u2014 open dashboard and login again"}:{error:E.message||`Request failed (${k.status})`}}f();q();L();var x=[],W=null,T=I(),_=null,K="vn_chat_sessions",be={model:{args:!0,hint:"/model <name|reset|?>"},clear:{args:!1,hint:"/clear"},page:{args:!1,hint:"/page"},images:{args:!1,hint:"/images"},sync:{args:!1,hint:"/sync"},snippets:{args:!1,hint:"/snippets"},help:{args:!1,hint:"/help"}},fe=["groq","llama","devstral","mistral","gemini","gemini-lite","gemma-26","gemma-31","reset"];function ut(){let e=document.getElementById("vn-tab-agent");e&&(e.innerHTML=`
...
`).filter(n=>t.test(n)).length>=2}function xn(e){return e.replace(/\|--/g,"\u251C\u2500\u2500 ").replace(/\+--/g,"\u251C\u2500\u2500 ").replace(/`--/g,"\u2514\u2500\u2500 ").replace(/\|  /g,"\u2502   ")}function yn(e){let t=document.getElementById("vn-inject-banner"),n=document.getElementById("vn-inject-text");t&&n&&(n.textContent=e.slice(0,180),t.classList.add("vn-show"))}function mt(){document.getElementById("vn-inject-banner")?.classList.remove("vn-show"),W=null}function wn(){let e=W||document.getElementById("vn-inject-text")?.textContent;if(!e)return;let t=En(e);mt(),W=null,t===!1?(d("red","no input found"),y("No writable input found on this page")):d("green",t==="clipboard"?"copied to clipboard":"injected \u2197")}function En(e){if(!e||typeof e!="string")return!1;let t=["textarea:not([disabled]):not([readonly])",'div[contenteditable="true"]','input[type="text"]:not([disabled])','[role="textbox"]'];for(let n of t){let o=document.querySelector(n);if(o)try{if(o.tagName==="TEXTAREA"||o.tagName==="INPUT")return o.focus(),o.value=e,o.dispatchEvent(new Event("input",{bubbles:!0})),o.dispatchEvent(new Event("change",{bubbles:!0})),!0;if(o.contentEditable==="true")return o.focus(),document.execCommand("selectAll",!1,null),document.execCommand("insertText",!1,e),!0}catch{continue}}try{return navigator.clipboard.writeText(e),"clipboard"}catch{return!1}}function Sn(){document.getElementById("vn-history-drawer")?.classList.add("vn-open"),gt()}function xe(){document.getElementById("vn-history-drawer")?.classList.remove("vn-open")}function _n(){x=[],Promise.resolve().then(()=>(f(),R)).then(({resetSession:t,getSessionId:n})=>{t(),T=n(),he()});let e=document.getElementById("vn-convo");e&&(e.innerHTML=`
    <div class="vn-msg vn-msg-agent">
      <div class="vn-msg-label">NExY</div>
