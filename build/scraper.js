...
`;function G(){if(document.getElementById("__venom_styles__"))return;let e=document.createElement("style");e.id="__venom_styles__",e.textContent=[F,J,U,X,Z,K,W,V].join(`
`),document.head.appendChild(e)}var Q="__venom_fab__",A="__venom_panel__",z=!1;function ee(){G();let e=document.createElement("div");e.id=Q,e.innerHTML='\u{1F577}\uFE0F<span class="vn-fab-dot"></span>';let t=document.createElement("div");return t.id=A,t.innerHTML=`
  ...
  `,De(),ce())}function De(){document.getElementById("vn-send-btn")?.addEventListener("click",ae),document.getElementById("vn-suggest-btn")?.addEventListener("click",Re),document.getElementById("vn-inject-btn")?.addEventListener("click",We),document.getElementById("vn-inject-dismiss")?.addEventListener("click",le),document.getElementById("vn-hamburger")?.addEventListener("click",Ge),document.getElementById("vn-drawer-close")?.addEventListener("click",O),document.getElementById("vn-new-chat")?.addEventListener("click",Qe),document.getElementById("vn-agent-input")?.addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),ae())})}async function ae(){let e=document.getElementById("vn-agent-input"),t=e?.value?.trim();if(!t)return;m.push({role:"user",content:t}),_("user",t),e.value="",d("amber","thinking..."),u("model","thinking...","warn");let{reply:n,model:o,escalated:r,error:i}=await $(t,v(),m);if(i){_("agent",`\u26A0 ${i}`),d("red","error"),b(i),u("model","error","err"),m.push({role:"assistant",content:`\u26A0 ${i}`});return}m.push({role:"assistant",content:n}),_("agent",n,o),d("green",r?"done (escalated)":"done"),u("model",o||"lite","active"),et(m)}async function Re(){let e="Based on this page, suggest a concise prompt I can send to my AI assistant. Reply with only the suggested prompt, nothing else.";d("amber","generating...");let{reply:t,error:n}=await $(e,v(),m);if(n||!t){d("red","suggestion failed"),b("Suggestion failed: "+(n||"empty reply"));return}k=t,Ke(t),d("green","ready to inject")}function _(e,t,n){let o=document.getElementById("vn-convo");if(!o)return;let r=document.createElement("div");r.className=`vn-msg vn-msg-${e}`;let i=document.createElement("div");i.className="vn-msg-label",i.textContent=e==="user"?"YOU":n?`NExY \xB7 ${n}`:"NExY",r.appendChild(i),Ye(t).forEach(s=>r.appendChild(s)),o.appendChild(r),o.scrollTop=o.scrollHeight}function Ye(e){let t=[];return e.split(/(```[\s\S]*?```)/g).forEach(o=>{if(o.startsWith("```")){let r=o.slice(3,-3),i=r.indexOf(`
`),s=i>=0?r.slice(0,i).trim():"",a=i>=0?r.slice(i+1).trim():r.trim();t.push(qe(s,a))}else if(Xe(o)){let r=document.createElement("pre");r.className="vn-ascii-block",r.textContent=Ze(o.trim()),t.push(r)}else if(o.trim()){let r=document.createElement("span");r.textContent=o,r.style.whiteSpace="pre-wrap",t.push(r)}}),t}function qe(e,t){let n=Ue(e,t),o=document.createElement("div");o.className="vn-code-block";let r=Fe(t,n);return o.innerHTML=`
    <div class="vn-code-header">
      <span class="vn-code-lang">#${n}</span>
      <button class="vn-code-copy">copy</button>
    </div>
    <div class="vn-code-body">${r}</div>
  `,o.querySelector(".vn-code-copy").onclick=()=>{navigator.clipboard?.writeText(t).catch(()=>{});let i=o.querySelector(".vn-code-copy");i.textContent="copied!",setTimeout(()=>{i&&(i.textContent="copy")},1500)},o}function Fe(e,t){let n=ve(e);return Je(t).forEach(([r,i])=>{n=n.replace(r,s=>`<span class="sh-${i}">${s}</span>`)}),n}function Je(e){let t=[/(&quot;|&#x27;)(.*?)(\1)/g,"str"],n=[/\/\/[^\n]*/g,"cmt"],o=[/#[^\n]*/g,"cmt"],r=[/\b(\d+\.?\d*)\b/g,"num"],i=[/\b(true|false|null|undefined|None|True|False)\b/g,"kw2"];return["js","ts","jsx","tsx"].includes(e)?[n,t,[/\b(const|let|var|function|return|if|else|for|while|class|import|export|from|async|await|new|typeof|instanceof|this|super|throw|try|catch|finally|of|in|switch|case|break|continue|default)\b/g,"kw"],i,r,[/\b([A-Z][a-zA-Z0-9_]*)\b/g,"cls"],[/\b([a-zA-Z_$][a-zA-Z0-9_$]*)\s*(?=\()/g,"fn"]]:["py","python"].includes(e)?[o,t,[/\b(def|class|import|from|return|if|elif|else|for|while|in|not|and|or|is|with|as|try|except|finally|raise|lambda|pass|break|continue|yield|global|nonlocal)\b/g,"kw"],i,r,[/\b([A-Z][a-zA-Z0-9_]*)\b/g,"cls"],[/\b([a-z_][a-zA-Z0-9_]*)\s*(?=\()/g,"fn"]]:["sh","bash","shell","zsh"].includes(e)?[o,t,[/\b(if|then|else|elif|fi|for|do|done|while|case|esac|function|return|export|source|echo|cd|ls|mkdir|rm|cp|mv|cat|grep|awk|sed|curl|git|npm|node)\b/g,"kw"],[/\$[a-zA-Z_][a-zA-Z0-9_]*/g,"var"],r]:e==="json"?[t,[/\b(true|false|null)\b/g,"kw2"],r]:e==="css"?[[/\/\*[\s\S]*?\*\//g,"cmt"],t,[/([a-zA-Z-]+)\s*:/g,"kw"],[/#[0-9a-fA-F]{3,6}\b/g,"num"],r]:e==="html"?[[/&lt;!--[\s\S]*?--&gt;/g,"cmt"],[/&lt;\/?([a-zA-Z][a-zA-Z0-9-]*)/g,"kw"],t]:[t,r,i]}function Ue(e,t){let n=(e||"").toLowerCase().trim(),o={js:"js",javascript:"js",jsx:"jsx",ts:"ts",typescript:"ts",tsx:"tsx",py:"py",python:"py",sh:"sh",bash:"sh",shell:"sh",zsh:"sh",json:"json",css:"css",html:"html",sql:"sql",yaml:"yaml",yml:"yaml",md:"md",markdown:"md",cpp:"cpp",c:"c",java:"java",rs:"rs",go:"go",rb:"rb"};return o[n]?o[n]:/^\s*\{[\s\S]*\}\s*$/.test(t)&&/"[^"]+"\s*:/.test(t)?"json":/^<(!DOCTYPE|html|head|body|div|span|p )/i.test(t.trim())?"html":/(def |import |from .* import|print\(|elif |:\s*$)/m.test(t)?"py":/(const |let |var |function |=>|require\(|module\.exports)/m.test(t)?"js":/(#!\/bin\/(bash|sh)|echo |grep |awk |sed |curl |npm |git )/m.test(t)?"sh":/SELECT|INSERT|UPDATE|DELETE|FROM|WHERE/i.test(t)?"sql":/^\s*[\w-]+\s*:/m.test(t)&&!/{/.test(t)?"yaml":/^(\/\/|\/\*|#include|using namespace|public class|fn |impl )/m.test(t)?"code":n||"code"}function Xe(e){let t=/[├└│─┤┬┴┼]/;return e.trim().split(`
`).filter(o=>t.test(o)).length>=2}function Ze(e){return e.replace(/\|--/g,"\u251C\u2500\u2500 ").replace(/\+--/g,"\u251C\u2500\u2500 ").replace(/`--/g,"\u2514\u2500\u2500 ").replace(/\|  /g,"\u2502   ")}function Ke(e){let t=document.getElementById("vn-inject-banner"),n=document.getElementById("vn-inject-text");t&&n&&(n.textContent=e.slice(0,180),t.classList.add("vn-show"))}function le(){document.getElementById("vn-inject-banner")?.classList.remove("vn-show"),k=null}function We(){let e=k||document.getElementById("vn-inject-text")?.textContent;if(!e)return;let t=Ve(e);le(),k=null,t===!1?(d("red","no input found"),b("No writable input found on this page")):d("green",t==="clipboard"?"copied to clipboard":"injected \u2197")}function Ve(e){if(!e||typeof e!="string")return!1;let t=["textarea:not([disabled]):not([readonly])",'div[contenteditable="true"]','input[type="text"]:not([disabled])','[role="textbox"]'];for(let n of t){let o=document.querySelector(n);if(o)try{if(o.tagName==="TEXTAREA"||o.tagName==="INPUT")return o.focus(),o.value=e,o.dispatchEvent(new Event("input",{bubbles:!0})),o.dispatchEvent(new Event("change",{bubbles:!0})),!0;if(o.contentEditable==="true")return o.focus(),document.execCommand("selectAll",!1,null),document.execCommand("insertText",!1,e),!0}catch{continue}}try{return navigator.clipboard.writeText(e),"clipboard"}catch{return!1}}function Ge(){document.getElementById("vn-history-drawer")?.classList.add("vn-open"),ce()}function O(){document.getElementById("vn-history-drawer")?.classList.remove("vn-open")}function Qe(){m=[],I=Date.now();let e=document.getElementById("vn-convo");e&&(e.innerHTML=`
      <div class="vn-msg vn-msg-agent">
        <div class="vn-msg-label">NExY</div>
        <span>Ask me anything about this page...</span>
      </div>
    `);let t=document.getElementById("vn-session-label");t&&(t.textContent="NEW SESSION"),O()}function et(e){if(!(!e||e.length===0))try{let t=JSON.parse(localStorage.getItem(L)||"[]"),n=t.findIndex(i=>i.id===I),o=e.find(i=>i.role==="user")?.content?.slice(0,60)||"Chat",r={id:I,ts:new Date().toLocaleTimeString(),preview:o,messages:e.slice()};n>=0?t[n]=r:t.unshift(r),localStorage.setItem(L,JSON.stringify(t.slice(0,20)))}catch{}}function ce(){let e=document.getElementById("vn-history-list");if(e)try{let t=JSON.parse(localStorage.getItem(L)||"[]");if(t.length===0){e.innerHTML='<div class="vn-history-empty">No sessions yet</div>';return}e.innerHTML=t.map(n=>`
    ...
    `).join(""),e.querySelectorAll(".vn-history-item").forEach(n=>{n.onclick=()=>tt(Number(n.dataset.id))})}catch{}}function tt(e){try{let n=JSON.parse(localStorage.getItem(L)||"[]").find(i=>i.id===e);if(!n)return;m=n.messages.slice(),I=n.id;let o=document.getElementById("vn-convo");if(!o)return;o.innerHTML="",m.forEach(i=>{_(i.role==="user"?"user":"agent",i.content)});let r=document.getElementById("vn-session-label");r&&(r.textContent=n.preview.slice(0,20)+"..."),O()}catch{}}function ve(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#x27;")}var pe=null;function ue(e){document.addEventListener("selectionchange",()=>{clearTimeout(pe),pe=setTimeout(()=>{let t=B();t&&e(t)},300)}),document.addEventListener("touchend",()=>{setTimeout(()=>{let t=B();t&&e(t)},50)})}function B(){let e=window.getSelection();if(!e||e.rangeCount===0)return"";let t=e.toString().trim();return t.length>0?t.slice(0,2e3):""}var me="https://vinnsedesigner-vinns-ai-backend.hf.space";async function ge(e){let t=p(),n=v();if(!t)return e("error","Login to dashboard first"),!1;if(n.length===0)return e("error","No snippets staged"),!1;e("syncing",`Syncing ${n.length} snippets...`);try{let o=await fetch(`${me}/api/scraper-agent`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify({sessionId:y(),snippets:n.map(i=>({number:i.number,text:i.text,type:i.type,url:i.url||location.href,title:i.title||document.title})),pageContext:S()})}),r=await o.json();return o.ok?(E(),e("success",`\u2713 ${r.saved} snippets synced`),!0):(e("error",r.message||"Sync failed"),!1)}catch{return e("error","Backend unreachable \u2014 buffered locally"),!1}}async function fe(e){if(!e)return null;try{let t=await fetch(`${me}/api/sync`,{headers:{Authorization:`Bearer ${e}`}});return t.ok?(await t.json()).settings:null}catch{return null}}var c=null,f=[];function xe(){let e=document.getElementById("vn-tab-snippets");e&&(e.innerHTML=`
  ...
  `,rt(),nt(),it(),g())}function nt(){document.getElementById("vn-modal-close")?.addEventListener("click",be),document.getElementById("vn-view-modal")?.addEventListener("click",e=>{e.target.id==="vn-view-modal"&&be()})}function ot(e){let t=document.getElementById("vn-view-modal"),n=document.getElementById("vn-modal-title"),o=document.getElementById("vn-modal-body");!t||!o||(n.textContent=`#${e.number} \xB7 ${e.type} \xB7 ${e.title||"snippet"}`,o.textContent=e.text,t.classList.add("vn-show"))}function be(){document.getElementById("vn-view-modal")?.classList.remove("vn-show")}function rt(){c||(c=document.createElement("div"),c.className="vn-sel-overlay",c.innerHTML=`
  ...
  `,document.body.appendChild(c),c.querySelector("#vn-sel-repo").onclick=()=>he("code"),c.querySelector("#vn-sel-research").onclick=()=>he("research"),document.addEventListener("click",e=>{c.contains(e.target)||M()}))}function ye(e){!c||!e||setTimeout(()=>{let t=window.getSelection();if(!t||t.rangeCount===0)return;let n=t.getRangeAt(0).getBoundingClientRect();c.style.top=`${n.bottom+8}px`,c.style.left=`${Math.max(0,n.left)}px`,c.classList.add("vn-show")},300)}function M(){c?.classList.remove("vn-show")}function he(e){let t=B();if(!t)return;let n=v(),o=parseInt(localStorage.getItem("vn_snippet_limit")||"20");if(n.length>=o){d("red",`Max ${o} snippets`),b(`Snippet limit (${o}) reached \u2014 sync or clear first`),M();return}Y({text:t,type:e,url:location.href,title:document.title}),g(),M(),d("green",`#${n.length+1} staged`)}function it(){document.getElementById("vn-sync-btn")?.addEventListener("click",async()=>{await ge((e,t)=>{d(e==="success"?"green":e==="error"?"red":"amber",t),e==="error"&&b(t),e==="success"&&g()})}),document.getElementById("vn-undo-btn")?.addEventListener("click",at)}function st(e){let n=v().find(r=>r.number===e);if(!n)return;q(e),g();let o={snippet:n,timer:null};f.push(o),we(`Snippet #${e} removed`),o.timer=setTimeout(()=>{f=f.filter(r=>r.snippet.number!==e),f.length===0&&Ee()},5e3)}function at(){let e=f.pop();if(!e)return;clearTimeout(e.timer);let t=v();t.push(e.snippet),t.sort((n,o)=>n.number-o.number),w(t),g(),f.length===0?Ee():we(`Snippet #${f[f.length-1].snippet.number} removed`)}function we(e){let t=document.getElementById("vn-undo-bar"),n=document.getElementById("vn-undo-text");t&&n&&(n.textContent=e,t.classList.add("vn-show"))}function Ee(){document.getElementById("vn-undo-bar")?.classList.remove("vn-show")}function g(){let e=v(),t=e.filter(s=>s.type==="code"),n=e.filter(s=>s.type==="research"),o=document.getElementById("vn-repo-list"),r=document.getElementById("vn-research-list");document.getElementById("vn-repo-count")&&(document.getElementById("vn-repo-count").textContent=t.length),document.getElementById("vn-research-count")&&(document.getElementById("vn-research-count").textContent=n.length),u("snippets",`${e.length} snippets`,e.length>0?"active":"");let i=(s,a)=>{s&&(s.innerHTML=a.length===0?'<div class="vn-snip-empty">\u2014</div>':a.map(dt).join(""),s.querySelectorAll(".vn-snip-view").forEach(l=>{l.onclick=()=>{let x=e.find($e=>$e.number===Number(l.dataset.num));x&&ot(x)}}),s.querySelectorAll(".vn-snip-del").forEach(l=>{l.onclick=()=>st(Number(l.dataset.num))}))};i(o,t),i(r,n)}function dt(e){return`
    <div class="vn-snip-item">
      <span class="vn-snip-num">#${e.number}</span>
      <span class="vn-snip-preview">${lt(e.text.slice(0,45))}</span>
  ...
  `}function lt(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function _e(){let e=document.getElementById("vn-tab-settings");if(!e)return;let t=parseInt(localStorage.getItem("vn_panel_height")||"55"),n=localStorage.getItem("vn_theme")||"oled",o=parseInt(localStorage.getItem("vn_snippet_limit")||"20");e.innerHTML=`
  ...
  `,ct()}function ct(){let e=document.getElementById("vn-height-slider"),t=document.getElementById("vn-height-val"),n=document.getElementById("vn-theme-oled"),o=document.getElementById("vn-theme-light"),r=document.getElementById("vn-snippet-limit"),i=document.getElementById("vn-clear-snippets");e?.addEventListener("input",()=>{let s=e.value;t.textContent=`${s}vh`;let a=h();a&&(a.style.height=`${s}vh`),localStorage.setItem("vn_panel_height",s)}),n?.addEventListener("click",()=>{Se("oled"),n.classList.add("vn-selected"),o?.classList.remove("vn-selected")}),o?.addEventListener("click",()=>{Se("light"),o.classList.add("vn-selected"),n?.classList.remove("vn-selected")}),r?.addEventListener("change",()=>{let s=Math.max(1,Math.min(100,parseInt(r.value)||20));localStorage.setItem("vn_snippet_limit",s),r.value=s}),i?.addEventListener("click",()=>{confirm("Clear all snippets?")&&(E(),g())})}function Se(e){let t=h();t&&(t.classList.remove("vn-theme-oled","vn-theme-light"),e!=="oled"&&t.classList.add(`vn-theme-${e}`),localStorage.setItem("vn_theme",e))}var T=!1,ke,Ie,H,P,C=!1,Le,Be;function Te(){vt(),pt()}function vt(){let e=j();if(!e)return;function t(r,i){T=!0,ke=r,Ie=i;let s=e.getBoundingClientRect();H=s.left,P=s.top,e.style.right="auto",e.style.bottom="auto",e.style.left=H+"px",e.style.top=P+"px",e.style.transition="none",e.classList.add("vn-dragging")}function n(r,i){if(!T)return;let s=r-ke,a=i-Ie,l=Math.max(0,Math.min(window.innerWidth-56,H+s)),x=Math.max(0,Math.min(window.innerHeight-56,P+a));e.style.left=l+"px",e.style.top=x+"px",localStorage.setItem("vn_fab_left",l),localStorage.setItem("vn_fab_top",x)}function o(){T=!1,e.classList.remove("vn-dragging")}e.addEventListener("mousedown",r=>{r.target!==e&&!r.target.classList.contains("vn-fab-dot")||(r.preventDefault(),r.stopPropagation(),t(r.clientX,r.clientY))}),document.addEventListener("mousemove",r=>n(r.clientX,r.clientY)),document.addEventListener("mouseup",o),e.addEventListener("touchstart",r=>{if(r.target!==e&&!r.target.classList.contains("vn-fab-dot"))return;let i=r.touches[0];t(i.clientX,i.clientY)},{passive:!0}),document.addEventListener("touchmove",r=>{if(!T)return;r.preventDefault();let i=r.touches[0];n(i.clientX,i.clientY)},{passive:!1}),document.addEventListener("touchend",o)}function pt(){let e=document.getElementById("vn-drag-handle"),t=h();if(!e||!t)return;function n(i){C=!0,Le=i,Be=t.offsetHeight}function o(i){if(!C)return;let s=Le-i,a=Math.max(window.innerHeight*.25,Math.min(window.innerHeight*.85,Be+s));t.style.height=a+"px",localStorage.setItem("vn_panel_height",Math.round(a/window.innerHeight*100))}function r(){C=!1}e.addEventListener("mousedown",i=>{i.preventDefault(),n(i.clientY)}),document.addEventListener("mousemove",i=>o(i.clientY)),document.addEventListener("mouseup",r),e.addEventListener("touchstart",i=>{n(i.touches[0].clientY)},{passive:!0}),document.addEventListener("touchmove",i=>{C&&(i.preventDefault(),o(i.touches[0].clientY))},{passive:!1}),document.addEventListener("touchend",r)}function Ce(){let e=j();if(!e)return;let t=localStorage.getItem("vn_fab_left"),n=localStorage.getItem("vn_fab_top");t&&n&&(e.style.right="auto",e.style.bottom="auto",e.style.left=t+"px",e.style.top=n+"px")}function Ne(){let e=h();if(!e)return;let t=parseInt(localStorage.getItem("vn_panel_height")||"55");e.style.height=`${t}vh`}var ut=15e3,D=null;function ze(){D=setInterval(async()=>{if(!document.getElementById("__venom_panel__")){clearInterval(D),D=null;return}let e=p(),t=await fe(e);t&&(t.snippet_limit&&localStorage.setItem("vn_snippet_limit",t.snippet_limit),g())},ut)}function Ae(){document.getElementById("__venom_fab__")||(ee(),ne(),de(),xe(),_e(),Te(),Ce(),Ne(),re(),ue(e=>ye(e)),se(),ze(),d("green","ready"))}function je(){let e=p();y(),Ae(),e||console.log("[VENOM Scraper] No auth token \u2014 open dashboard and login first")}(function(){"use strict";if(window.__nexus_scraper_loaded__){document.getElementById("__venom_fab__")?.remove(),document.getElementById("__venom_panel__")?.remove(),document.getElementById("__venom_styles__")?.remove(),window.__nexus_scraper_loaded__=!1;return}window.__nexus_scraper_loaded__=!0,je()})();})();
