...
export function initHud() {
  ...
  // Selection listener
  initSelection((text) => {
    showSelectionBar(text);
  });

  // Settings poll every 15s
  startPoll();

  // Initial snippet render
  renderSnippets();
}
...
function buildHud() {
  ...
  hud.innerHTML = `
    ...
    <div class="nx-body" id="nx-body">
      ...
      <!-- Inject banner (shown when agent has suggestion) -->
...
function renderSnippets() {
  ...
  list.innerHTML = snippets.map((s) => `
    <div class="nx-snippet-item">
      ...
      <span class="nx-snippet-preview">${escape(s.text.slice(0, 60))}...</span>
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    #__nexus_hud__ .nx-snippet-num  { color: #7c3aed; font-size: 10px; min-width: 24px; }
